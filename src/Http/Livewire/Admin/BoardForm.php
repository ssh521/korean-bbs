...
class BoardForm extends Component
{
    ...
    public function mount(?Board $board = null): void
    {
        if ($board && $board->exists) {
            ...
            $this->listLevel    = $board->list_level;
            $this->readLevel    = $board->read_level;
            $this->writeLevel   = $board->write_level;
            $this->commentLevel = $board->comment_level;
            $this->fileLevel    = $board->file_level;
            $this->uploadLevel  = $board->upload_level;
            $this->downloadLevel = $board->download_level;
            $this->likeLevel    = $board->like_level;
    ...
    public function save(): void
    {
        $this->validate([
            ...
            'listLevel'    => 'required|integer|min:0|max:10',
            'readLevel'    => 'required|integer|min:0|max:10',
            'writeLevel'   => 'required|integer|min:0|max:10',
            'commentLevel' => 'required|integer|min:0|max:10',
            'uploadLevel'  => 'required|integer|min:0|max:10',
            'downloadLevel' => 'required|integer|min:0|max:10',
            'likeLevel'    => 'required|integer|min:0|max:10',
        ...
        $data = [
            ...
            'file_level'    => $this->uploadLevel,
            'upload_level'  => $this->uploadLevel,
            'download_level' => $this->downloadLevel,
            'like_level'    => $this->likeLevel,
